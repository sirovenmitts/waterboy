...
;(function ($) {
	...
	// Popover template
	var tpl = Blaze.toHTMLWithData(Template.ClockPicker, {})
	...
	function ClockPicker(element, options) {
		...
		// disable click listener if input is disabled
		addon.on('click.clockpicker', function () {
			if (!addon.parent().find('input').prop('disabled')) {
	...
	/**
	 * Find most suitable vertical placement, doing our best to ensure it is
	 * inside of the viewport.
	 *
	 * First try to place the element according with preferredPlacement, then try
	 * the opposite placement and as a last resort, popover will be placed on the
	 * very top of the viewport.
	 *
	 * @param {jQuery} element
	 * @param {jQuery} popover
	 * @param preferredPlacement Preferred placement, if there is enough room for
	 *   it.
	...
	ClockPicker.DEFAULTS = {
		...
		ampmSubmit: false,	// allow submit with AM and PM buttons instead of the
											 // minute selection/picker
	...
	ClockPicker.prototype.locate = function () {
		...
		if (placement === 'top-adaptive' || placement === 'bottom-adaptive') {
			...
			// Adaptive placement should be resolved into one of the "static"
			// placement options, that is best suitable for the current window scroll
			// position.
	...
	ClockPicker.prototype.parseInputValue = function () {
		...
		if (this.options.twelvehour) {
			var period = (value[1] + '').replace(/\d+/g, '').toLowerCase().trim()
	...
	ClockPicker.prototype.show = function (e) {
		...
		// Hide when clicking or tabbing on any element except the clock, input and
		// addon
	...
	$.fn.clockpicker = function (option) {
		...
		// If we explicitly do a call on a single element then we can return the
		// value (if needed) This allows us, for example, to return the value of
		// getTime
		if (this.length == 1) {
			var returnValue = handleClockPickerRequest.apply(this[0])

			// If we do not have any return value then return the object itself so
			// you can chain
